;(function (win, doc) {
  ...
  function init (options) {
    ...
    config = typeof config !== 'undefined' ? options : null;
    checker = checkDOM();

    // add event listeners for subsequent checks
    addEvent(win, 'load', checker);
    addEvent(win, 'scroll', checker);
  }
  ...
  function checkDOM () {
    return debounce(function (e) {
      ...
      if (!!config && 'querySelectorAll' in doc) {
        anchors = ('container' in config && !!config.container ? doc.querySelector(config.container) : doc).querySelectorAll(('selector' in config && !!config.selector ? config.selector : 'a'));
      }
      ...
      if (typeof e === 'object' && !!e && 'type' in e && !!e.type) {
        if (e.type === 'load') {
          removeEvent(win, 'load', checker);
        }
        if (e.type === 'scroll' && f < 1) {
          removeEvent(win, 'scroll', checker);
        }
      }
      ...
      for (; g < f; ++g) {
        ...
        if (typeof anchors[g] === 'undefined' || !anchors[g].getAttribute('href') || anchors[g].getAttribute('href').length < 5 || !isInViewport(anchors[g])) {
  ...
  function processUrl (_link) {
    ...
    var _url = _link.href || _link.getAttribute('href'); // cache href contents
